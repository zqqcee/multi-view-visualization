...
export default function Canvas() {
    ...
    const initCanvasByData = (nodes, links) => {
        ...
        const renderRefresh = () => {
            ...
            //func2
            // const kx = width / (d3.max(nodes.map(node => node.x)) - d3.min(nodes.map(node => node.x))) > 0 ? width / (d3.max(nodes.map(node => node.x)) - d3.min(nodes.map(node => node.x))) : (d3.max(nodes.map(node => node.x)) - d3.min(nodes.map(node => node.x))) / width
            // const ky = height / (d3.max(nodes.map(node => node.y)) - d3.min(nodes.map(node => node.y))) > 0 ? height / (d3.max(nodes.map(node => node.y)) - d3.min(nodes.map(node => node.y))) : (d3.max(nodes.map(node => node.x)) - d3.min(nodes.map(node => node.x))) / height
            // const k = Math.min(kx, ky)
            // console.log(k);
            // ctx.translate(width * k, height * k);
            // ctx.scale(k, k);
